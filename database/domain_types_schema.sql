-- Domain-specific fallback types schema addition for Supabase
-- Add these tables to store domain-specific entity and relationship types
...
CREATE POLICY "Enable all access for development" ON domain_entity_types FOR ALL USING (true);
CREATE POLICY "Enable all access for development" ON domain_relationship_types FOR ALL USING (true);
CREATE POLICY "Enable all access for development" ON domain_analysis_cache FOR ALL USING (true);
CREATE POLICY "Enable all access for development" ON verb_extractions FOR ALL USING (true);
...
SELECT 
    ...
    (SELECT COUNT(DISTINCT relationship_type) 
     ...
     WHERE drt.domain = det.domain AND drt.subdomain = det.subdomain) as relationship_types_count,
