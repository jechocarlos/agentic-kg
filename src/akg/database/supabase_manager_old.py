...
class SupabaseManager:
    ...
    async def get_document_stats(self) -> Dict[str, int]:
        ...
        # Get total documents
        total_result = self.client.table('documents').select('id', count='exact').execute()
        total_count = total_result.count or 0
        
        # Get documents by status
        uploaded_result = self.client.table('documents').select('id', count='exact').eq('status', 'uploaded').execute()
        uploaded_count = uploaded_result.count or 0
        
        processed_result = self.client.table('documents').select('id', count='exact').eq('status', 'processed').execute()
        processed_count = processed_result.count or 0
        
        failed_result = self.client.table('documents').select('id', count='exact').eq('status', 'failed').execute()
...
class SupabaseManager:
    ...
    async def initialize(self):
        """Initialize Supabase client"""
        try:
            # Use environment variable to disable SSL verification
            import os
            os.environ['PYTHONHTTPSVERIFY'] = '0'
